...
CI runs the full Docker-backed Maven test suite, including ArchUnit boundaries, packages the application, discovers the resulting executable JAR without hard-coded filenames, lints the tracked wiki, and builds the container image. Requirement workspaces remain ignored local planning artifacts and are linted only during local work. Pinned OWASP Dependency Check runs when Maven dependencies change and on the weekly schedule. The optional `NVD_API_KEY` repository secret enables direct NVD API updates; when it is absent, the workflow omits API-key configuration and bootstraps from Dependency-Check's daily best-effort NVD data feed instead of the unreliable anonymous API. Dependabot covers Maven, Docker, and GitHub Actions; the Maven wrapper verifies its downloaded distribution checksum.
