...
CI runs the full Docker-backed Maven test suite, including ArchUnit boundaries, packages the application, discovers the resulting executable JAR without hard-coded filenames, lints the tracked wiki, and builds the container image. Requirement workspaces remain ignored local planning artifacts and are linted only during local work. Pinned OWASP Dependency Check runs when Maven dependencies or its suppression file change and on the weekly schedule. The optional `NVD_API_KEY` repository secret enables direct NVD API updates; when it is absent, the workflow omits API-key configuration and bootstraps from Dependency-Check's daily best-effort NVD data feed instead of the unreliable anonymous API.

Dependency suppressions must identify both the exact package family and vulnerability, explain why the advisory does not apply, and cite supporting advisory evidence. The scan fails on unused suppression rules so obsolete exceptions are removed. `pom.xml` temporarily overrides several Spring Boot 4.1.0 dependency-management properties with compatible security patch releases; remove each override after a Boot maintenance release manages an equal or newer version. The report may still contain sub-threshold findings in Testcontainers' test-scoped, shaded `docker-java-transport-zerodep`; those embedded libraries cannot be overridden by Maven and should be removed by the next upstream Testcontainers/docker-java release rather than suppressed as false positives. Dependabot covers Maven, Docker, and GitHub Actions; the Maven wrapper verifies its downloaded distribution checksum.
